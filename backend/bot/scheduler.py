...
async def start_scheduler():
    ...
    # ── Live data every minute, 9:00–15:30 IST ──
    _scheduler.add_job(
    ...
    _scheduler.add_job(
        job_intraday_snapshot,
        CronTrigger(minute="*/15", hour="9-15", day_of_week="mon-fri", timezone=IST),
        id="intraday_snapshot", replace_existing=True,
    )

    # ── Options signal check every 5 min, 9:45–14:30 IST ──
    _scheduler.add_job(
        job_check_options_signals,
        CronTrigger(minute="*/5", hour="9-14", day_of_week="mon-fri", timezone=IST),
        id="options_signals", replace_existing=True,
    )
...
async def job_check_options_signals():
    """Check if options signal conditions are met."""
    now = datetime.now(tz=IST)
    # Not before 9:45 AM, not after 2:30 PM
    ...
    try:
        ...
        await check_and_generate_signal()
    except Exception as exc:
        logger.error(f"job_check_options_signals error: {exc}", exc_info=True)


async def job_intraday_snapshot():
    try:
        await job_collect_live_data()
    except Exception as exc:
        logger.error(f"job_intraday_snapshot error: {exc}", exc_info=True)
...
async def job_broadcast_live():
    """Broadcast latest cached data to WebSocket clients every 5 seconds."""
    try:
        from websocket.live_feed import manager
        from db.connection import AsyncSessionLocal
        from db.models import DailyMarketSnapshot
        ...
        async with AsyncSessionLocal() as session:
            ...
            if snapshot and snapshot.all_data:
                await manager.broadcast_market_update(snapshot.all_data)
    except Exception:
        pass
